...
配置 `LOOMQ_LLM_BASE_URL`、`LOOMQ_LLM_API_KEY` 和 `LOOMQ_LLM_MODEL` 后，可直接运行：

```bash
python -m starter_kit.chat "生成一个 3 比特 GHZ 态并进行全测量"
```

该入口会原样展示 Agent 的解释与 QASM，适合现场演示和问题复现。
零基础用户可先阅读 [`BEGINNER_GUIDE.md`](BEGINNER_GUIDE.md)；CLI 会为回复中的 QASM 给出本地理想测量结果柱状图。

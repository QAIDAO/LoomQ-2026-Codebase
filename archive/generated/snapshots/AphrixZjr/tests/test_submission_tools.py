...
class ArchiveSafetyTests(unittest.TestCase):
    def test_extracts_only_starter_kit(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                (destination / "adapter.py").read_text(encoding="utf-8"), "ok"
            )
...
class CollectorEndToEndTests(unittest.TestCase):
    def test_collects_from_artifact_without_reading_source_fork(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(extracted.read_text(encoding="utf-8"), "collected")
            manifest = json.loads(
                (output / "intake-manifest.json").read_text(encoding="utf-8")
            )
            ...
            mismatch_manifest = json.loads(
                (mismatch_output / "intake-manifest.json").read_text(
                    encoding="utf-8"
                )
            )

...
> **目录名勘误**：流程图中的 `starter-kit/` 是早期名称。当前 Starter Kit v1.1.0、自动归档和正式评测唯一有效的目录名是 `starter_kit/`（下划线）；请以本 README、`competition/config.json` 和实际仓库目录为准。

流程图文字版（同样作为提交核对依据）：

1. 指定一个 GitHub 提交账号：GitHub 用户名即 Team ID，fork 所有者和最终提交 Issue 作者都必须是该账号。
2. 在 `starter_kit/` 内开发，并保留 `submission.yaml`、`adapter.py`、`Dockerfile`、`README.md`。
3. Commit 并 push 全部参赛代码，记录 40 位完整 commit SHA。
4. 运行 `python3 starter_kit/prepare_submission.py --team-id <GITHUB_USERNAME>`，确认本地预检通过。
5. 在上游创建“LoomQ 最终提交”Issue，填写 Team ID、fork 地址、commit SHA、Level；申报真机分时填写证据路径。
6. 等待自动校验。只有 `submission:accepted` 标签，以及包含归档 SHA-256 和 Artifact ID 的回执同时出现才算成功；被拒后应修复并新建 Issue，不要编辑旧 Issue。

截止时间为 **2026-08-25 12:00 UTC+8**，以 GitHub Issue 的 `created_at` 为准。代码更新后必须新建 Issue，截止前最后一次 accepted 提交生效。

...
本 fork 已实现 L1、L2、L3 和自定义量子 RISC-V 扩展。在 fork 根目录执行下面一条命令，无需安装依赖、注册账号或配置 API Key：

```bash
python3 starter_kit/loomq_cli.py --demo bell
```

如果组委会已将 `starter_kit/` 解包为当前评测根目录，等价命令为 `python3 loomq_cli.py --demo bell`。

输出同时包含可执行 QASM、shots 统计、精确百分比、无颜色依赖的 ASCII 直方图和人话解释。自然语言 Agent、三项体验任务、架构、错误恢复和完整验证命令见 [`SOLUTION.md`](SOLUTION.md)。
...
├── riscv_emulator.py
├── quantum_riscv_extension.md
├── test_quantum_riscv_extension.py
...
│   └── ghz3.qasm
└── examples/
    ├── spinq_cloud_submit.py
    ├── originq_cloud_submit.py
    └── run_*.py
```
...
公开 evaluator 只使用 Python 标准库，无需安装依赖。推荐 Python 3.10，与官方基础镜像一致（spinqit 最高只提供 cp310 wheel）。从组委会解包后的 `starter_kit` 评测根执行：

```bash
python3 evaluator.py --level l1 --target spinq,originq --json-out report.json
```

从 fork 根目录执行时，等价命令是 `(cd starter_kit && python3 evaluator.py --level l1 --target spinq,originq --json-out report.json)`。

参赛项目使用第三方 SDK 时，必须把依赖写入 `requirements.txt` 并精确锁定版本，例如 `package==1.2.3`。不要提交 `package>=1.2`，正式评测不会替参赛队选择依赖版本。

也可以先验证基础容器。从 `starter_kit` 评测根执行：
...
从 fork 根目录则执行 `docker build -t loomq-submission starter_kit && docker run --rm loomq-submission`。

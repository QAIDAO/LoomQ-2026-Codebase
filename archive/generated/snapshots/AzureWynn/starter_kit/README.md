...
LoomQ 是一个**量子接入平权**实现：把"会量子"从"看得懂 QASM"变成"会说中文"。

- **L1 通用中间层**：同一套 OpenQASM 2.0 电路，驱动量旋、本源、AWS Braket 三家（本地模拟器），位序、门集差异在中间层统一处理，返回一致的结果格式。
- **L2 智能体**：自然语言生成 / 修复 / 选后端，并自带"生成 → 本地模拟器自验 → 不对就重试"的闭环；附一个零基础 CLI 入口（`cli.py`）。
- **L3 混合编译**：把 Hybrid-QASM 的经典控制块编译成可跑的 RISC-V 汇编。
- **Bonus 量子 RISC-V**：在官方模拟器上扩展一组量子自定义指令（规格 + 实现 + 端到端测试）。

模块划分、设计思路与一键运行见 [`ARCHITECTURE.md`](ARCHITECTURE.md)。自动评测入口仍是本文件下方约定的 `adapter.py` 契约。

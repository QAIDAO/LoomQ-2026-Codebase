...
├── requirements.txt
├── Dockerfile
├── quantumhelper_web/       # L2 Agent 网页入口与本地 API 服务
├── evidence/
│   ├── README.md
│   └── files/                # 可选附件
├── circuits/
...
## QuantumHelper Web：L2 Agent 用户入口

网页源代码已经包含在正式提交目录的 `quantumhelper_web/` 中，直接调用本目录的
`adapter.py`。从仓库根目录启动：
...
如果当前目录就是评测归档解压后的 `starter_kit/`：

```bash
python quantumhelper_web/server.py
```

打开主工作台 <http://127.0.0.1:8000/>；`/agent.html` 仅用于兼容旧链接。启用真实 Agent 前必须设置
`LOOMQ_LLM_BASE_URL`、`LOOMQ_LLM_API_KEY`、`LOOMQ_LLM_MODEL`，并设置
`QUANTUMHELPER_ENABLE_LLM=1`。自然语言生成或修复得到的 QASM 会再次通过
`parse_qasm()` 与三个 L1 目标的 `transpile()` 校验；后端选择结果必须命中
`backend_capabilities.json` 中的规范 ID。

面向评审与部署的构建方式、主要模块、目标用户、完整使用流程、安全边界和验收命令，见
[`evidence/files/engineering-productization/engineering-productization.md`](evidence/files/engineering-productization/engineering-productization.md)。

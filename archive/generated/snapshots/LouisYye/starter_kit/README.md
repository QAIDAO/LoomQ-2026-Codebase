...
配置好环境变量后，可通过面向新手的交互式 CLI 连续使用 Agent：

```bash
python3 -m starter_kit.l2_cli
```

启动菜单内置生成 GHZ、修复 Bell 电路和按限制推荐后端三个示例。生成电路时，
CLI 还会展示本地模拟的 counts、概率和文本直方图；也可使用
`--example 1` 或 `--prompt "生成 Bell 态并测量"` 单次运行。
...
系统各模块的职责、一次请求的完整数据流、确定性验证边界、目标用户与 AI 辅助开发
说明见 [`ARCHITECTURE.md`](ARCHITECTURE.md)。

...
```text
干净环境中的构建和启动命令：pip install -r requirements.txt && python -m starter_kit.tests.test_transpile
架构说明：见 README.md，采用策略模式，BaseBackend 抽象类 + 三平台适配器（SpinQ/OriginQ/Braket）
目标用户和使用场景：不懂量子物理的普通开发者，用自然语言指挥量子计算机
完整使用流程：见 README.md 中的使用示例
```

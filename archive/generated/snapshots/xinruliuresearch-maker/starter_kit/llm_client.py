    ...
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
            ...
            if len(raw) > MAX_RESPONSE_BYTES:
                raise RuntimeError("LoomQ L2 API response exceeds the safety limit")
            return json.loads(raw)
    except urllib.error.HTTPError as exc:
        # HTTPError is also a file-like response; close it explicitly before
        # retaining the exception as the cause so failed calls never leak a
        # connection until garbage collection.
        status = exc.code
        exc.close()
        raise RuntimeError("LoomQ L2 API returned HTTP %d" % status) from exc
    except (urllib.error.URLError, TimeoutError) as exc:
        raise RuntimeError("LoomQ L2 API is unreachable") from exc
    except json.JSONDecodeError as exc:
        raise RuntimeError("LoomQ L2 API returned invalid JSON") from exc

...
python evaluator.py --level declared
```

第一条装依赖（三个平台 SDK + 锁定 antlr 版本）；第二条跑公开自测：
L1 公开电路转译与执行、L3 混合编译样例应全部 PASS。唯一例外：
**L2 自检需要 LLM 配置**——正式评测由组委会注入 `LOOMQ_LLM_*` 环境变量；
本地完整体验请先：

```bash
set LOOMQ_LLM_BASE_URL=https://api.deepseek.com/v1
set LOOMQ_LLM_API_KEY=sk-你的key
set LOOMQ_LLM_MODEL=deepseek-chat
```

未配置时 L2 一项显示 FAIL 属预期，不影响 L1/L3 自检。

- 想玩网页版量子Kitty：`python webapp.py` 后浏览器打开 `http://127.0.0.1:8765`
  （需设置 LOOMQ_LLM_* 环境变量；零配置可双击 `demo.html` 离线体验）。

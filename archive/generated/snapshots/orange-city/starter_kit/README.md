...
目标用户：没有量子物理背景、只会 Python 的开发者。他们不需要学会 OriginIR / QCIS，只要输入自然语言或标准 OpenQASM 2.0。

一键自测（在 `starter_kit/` 下，无需安装 SDK）：
...
架构：`adapter.py` 是唯一评测入口；`transpilers.py` 把 QASM 译到三平台原生 IR；`qasm_engine.py` 做无噪声模拟；`agent.py` 在组委会 LLM 上生成/纠错/选后端并用 L1 自验；`hybrid_compiler.py` 把 Hybrid-QASM 编成官方模拟器支持的 RISC-V。

平权叙事与 CLI 证据见 [`PROJECT_README.md`](PROJECT_README.md) 与 [`evidence/README.md`](evidence/README.md)。

...
用自然语言生成量子电路，也可以直接询问量子概念、数学逻辑和当前实验；然后在 SpinQ、OriginQ 和 AWS Braket 本地模拟器上运行，并通过电路、量子门解释和真实 counts 理解实验结果。

- Docker 一键 setup + run：`docker compose -f starter_kit/compose.yaml up --build`

...
这条命令构建 `Dockerfile.playground`、安装精确锁定的运行依赖并启动 `http://127.0.0.1:4173/`。没有 LLM 或远端硬件凭证时仍可打开产品并使用本地模拟器；正式 L2 评测由组委会按 `submission.yaml` 注入 `LOOMQ_LLM_BASE_URL`、`LOOMQ_LLM_API_KEY` 和 `LOOMQ_LLM_MODEL`。产品架构、组件边界和一键复现说明见 [`evidence/README.md`](evidence/README.md) 与 [`docs/PLAYGROUND_INTEGRATION.md`](docs/PLAYGROUND_INTEGRATION.md)。
